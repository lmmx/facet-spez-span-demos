...
// Default implementation for any format where SpanType = Raw
impl<F: Format<SpanType = Raw>> ToCooked<F> for Span<Raw> {
    fn to_cooked(self, format: &F) -> Span<Cooked> {
        println!("DEFAULT: Raw to Cooked conversion for format: {}", std::any::type_name::<F>());
        format.cook_span(self.start, self.len)
    }
}
    ...
    Span<F::SpanType>: ToCooked<F>,
{
    fn run(self, format: &F, _input: &str) -> Span<Cooked> {
        ...
        println!("HIGH PRIORITY: Using span with ToCooked<F> trait: {:?}", s);
        println!("  Format: {}", std::any::type_name::<F>());
        s.to_cooked(format)
    }
}

// Generic fallback implementation (low priority)
impl<F: Format> __Dispatch<F> for &&__Match<Span<F::SpanType>> {
    fn run(self, _format: &F, _input: &str) -> Span<Cooked> {
        let s = self.0.take().unwrap();
        println!("LOW PRIORITY: Generic fallback for span without ToCooked<F>: {:?}", s);
        println!("  Format: {}", std::any::type_name::<F>());
        // Default conversion
...
impl Format for CliFormat {
    ...
    fn cook_raw(&self, span: Span<Raw>) -> Span<Cooked> {
        println!("NOW COOKING: Raw to Cooked conversion for CliFormat");
